...
啥叫饿汉式？饿了就想立刻想到吃，类比到创建对象也是如此，当类一初始化，该类的对象就立刻会被实例化。
...
- 标注❷处是核心，`instance`使用`static`修饰，然后调用new创建对象，我们知道`static`修饰的东西都属于类，而且在类加载阶段就已经被加载，并且只能被加载一次。就是类加载这种特性很好的保证了单例的特性，也不会有天然防止并发的问题。

卧槽，单例模式居然如此简单，这么轻松就完成一个，这就算完事了？呵呵......

这个代码确实实现了单例的效果，只要调用`HungrySingleton.getInstance()`，你就算是神仙也造不出第二个对象......
...
没错，在单线程下面确实如此，可惜忽略了多线程场景。为什么在多线程下会有问题？分析一下，现在有两个线程A和B，同时到达❸处，自然此时`instance = new LazySingleton()`这一行都每调用过，自然❸处条件成立，然后A和B同时进入了`if{}`代码块，后面的事情就知道了，A和B线程都会调用`instance = new LazySingleton()`，从而创建多个对象，破坏了单例。
